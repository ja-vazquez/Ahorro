...
class Settings:
    def __init__(self, _person, _today_date, _invest):
        ## All settings: dirs, names, files, most of the info
        ## needed throught files
        self.person      = _person
        ...
        self.depos_file  = '{0}/{0}_Res_depos.txt'.format(self.person) 
        self.dir_month   = '{}{}/{}/'.format(self.directory, self.person, self.today_month)
        self.file_month  = '{}_{}'.format(self.person, self.today_month)
        if not os.path.isdir(self.dir_month): os.system('mkdir {}'.format(self.dir_month))
       
        self.file_finan  = '{}Finances.pdf' .format(self.pdf_dir)
        self.file_perform= '{}Perform.pdf'  .format(self.pdf_dir)
        self.plot_name   = 'Plots_{}.pdf'   .format(self.file_month)
        self.edo_name    = 'Edo_{}.csv'     .format(self.file_month)        
        self.month_day   = '{}_{}'          .format(self.today_month, self.today_day)
        self.file_people = '{}People_{}.pdf'.format(self.pdf_dir, self.month_day)
        
        if _invest == 'bursa':
            self.bursa_dir   = 'Investing/Bursanet/'
            self.bursa_info  = 'Investing.txt'
            self.file_invest = '{}Investing.pdf'.format(self.pdf_dir)
        else:
            self.bursa_dir   = 'Investing/Etoro/'
            self.bursa_info  = 'Etoro.txt'
            self.file_invest = '{}Etoro.pdf'.format(self.pdf_dir)
            
        self.file_bursa  = self.bursa_dir + self.bursa_info
            
            
    def traslation(self, month):
        ## Traslate month names to spanish
...
class Read_files:
    ...
    def all_persons(self):
        return pd.read_csv(self.Setts.directory + self.Setts.invest_info, sep='&', 
...
class Calculation:
    def __init__(self, person, today_date, _invest='bursa'):
        ## Main file for most of the calculations, people, invest, etc.. 
        self._person      = person
...
class Plotting():
    ...
    def _make_plot(self, ax, df, ylims, color='LimeGreen', label=None,
        ...
        df.plot.bar(color=color, label=label, stacked=True, alpha=0.8, ax=ax)
    ...
    def deposits_plot(self):
        ...
        if self.Calcul.df_deposits.index.tolist()[0] == '01/Jan':	
            ...
            self._make_plot(ax1, Deposits[:1], 
                 ...
                 color  = 'gold', ylabel = 'Depositos (MXN)')
            ...
            self._make_plot(ax3, self.Calcul.group_monthly[['cum_depos','cum_interest']],
                 ylims  = self.Calcul.group_monthly.cum_depos.max()*1.3, 
                 color  = ['blue','Magenta'], title  = 'Total acumulado',
...
class Run_all(Calculation):
    ...
    def all_result(self):
        ...
        total_invest.set_value('bursa', self.Rfiles.read_bursa(self.Setts.file_bursa)['total'].iloc[-1])
        ...
        plt.legend(['People: %.1f, %.1f%%'%(total_invest.gain_investors, 
                    ...
                    total_invest.gain_mine/my_depos*100.)], loc='best')
