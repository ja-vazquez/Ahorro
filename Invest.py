...
class Invest(Calculation):
	...
	#print df_invest
        ...
        #print df_invest.groupby([df_invest.index.month]).tail(1).money_in
    ...
    def anotatep(self, ax, text, xy, color='gold'):
        ax.annotate(text, xy=xy, xycoords='axes fraction', fontsize=18, 
                    horizontalalignment='right', bbox={'alpha': 0.4, 'pad': 10, 'facecolor': color}, 
    ...
    def plot_feat(self, ax):
        ...
        ax.grid(True)
        plt.xlim(datetime.date(self.Setts.today_year, 01, 01), 
                 self.extra_month()) #datetime.date(self.Setts.today_year + 1, 01, 01))
...
class Performance(Calculation):
	...
	ax1.set_title('Deposits: %s,  total: %s, earn: %s, Percentage: %.1f %%, rate: %.1f p/month'%(
        ...
        os.system("open -a Preview Performance.pdf")
        #plt.show(block=True)
